...
- Resolved Windows verification warning: a default temporary directory beneath an unrelated Git checkout used to make non-Git lifecycle tests inherit that checkout and create test state in its metadata. Storage tests now set `GIT_CEILING_DIRECTORIES` to their temporary root, which fences `git rev-parse`; enforcement tests additionally create a `.git` directory in their temporary consumer, which fences the installer's pure-Python ancestor walk. An empty `.git` does not fence `git` itself, so a test that resolves a lifecycle state root needs the ceiling as well. No `TEMP`/`TMP` preparation is needed.
