# Why the mechanics are what they are

This document is not distributed to consumers. `docs/agent-handoff/mechanics.md`
states what the toolkit enforces; this states why, and records the failures each
rule was written against. An agent reading a blocked session needs the former.
Anyone changing the toolkit needs both.
...
approval. The immediately following user response must approve the exact
proposal; ambiguous, qualified, negative, or non-adjacent replies leave the
proposal pending. This preserves inherited authority while allowing explicit,
approved transition evidence.

The mechanism stays in the code and out of the distributed reference. Across the
three consumers, 0 of 39 schema-v2 records carry a transition, and every word
about it was being loaded by sessions that would never use it. A session whose
goal has genuinely changed has a plainer path: say so, and let the user declare
the new goal with `Track:`.

## Host verification

`acceptance --platform <host>` is an opt-in local smoke check. It reports
`unverified` rather than `pass` unless trusted hook discovery is observed, and
from the command line it always reports `unverified`, because the trusted
evidence verifier is a test seam.

Observed on 2026-09-18:

- Claude Code: `discovered=pass` and `advisory_seen=pass`. The second settles an
  open question from the 2026-09-17 review — the model does receive
  `hookSpecificOutput.additionalContext` at `PreToolUse`, so `AHK-DECLARE` is
  correctly delivered there and does not need a `systemMessage` beside it.
- Codex: `discovered=fail`. The Codex account had no usage credits, so the host
  could not start a session at all and nothing could be observed. Codex
  end-to-end behaviour stays unverified, which is the outcome the convention
  requires when discovery is not observed.
